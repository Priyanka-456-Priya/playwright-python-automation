"""
Test Suite Runner for PRODUCTION Environment
Automatically switches to production environment and runs the full test suite

WARNING: This runs tests against PRODUCTION environment!
...
ENV_NAME = 'Production'
ENV_FILE = '.env.prod'
ENV_URL = 'https://api-iscrum.sureprep.com'
...
def print_warning():
    ...
    print_banner("!!! PRODUCTION ENVIRONMENT WARNING !!!", '*')
    print("  You are about to run tests against the PRODUCTION environment!")
    print("  This may affect live data and real users.")
    print("  Please ensure you have:")
    print("    1. Proper authorization to run tests in production")
    print("    2. Reviewed all test cases for production safety")
    print("    3. Notified relevant stakeholders")
    print(f"{'*'*80}\n")


def confirm_production_run():
    """Ask for user confirmation before running production tests"""
    print_warning()
    ...
    if response not in ['yes', 'y']:
        print("\n[ABORT] Production test execution cancelled by user.")
        return False

    print("\nPlease type 'RUN PRODUCTION TESTS' to confirm: ", end='')
    confirmation = input().strip()

    if confirmation != 'RUN PRODUCTION TESTS':
        print("\n[ABORT] Confirmation text did not match. Production test execution cancelled.")
        return False

    print("\n[CONFIRMED] Proceeding with production test execution...")
    return True
...
def run_tests():
    ...
    # Run pytest
    try:
        print_banner(f"RUNNING TESTS - {ENV_NAME} [PRODUCTION]", '=')
...
def main():
    """Main entry point"""
    print_banner(f"PRODUCTION Environment Test Suite Runner", '=')
    print(f"  Environment: {ENVIRONMENT.upper()}")
    ...
    print(f"  Environment: {ENVIRONMENT.upper()} [PRODUCTION]")
